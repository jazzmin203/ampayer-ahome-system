...
def create_users():
    ...
    for i in range(1, 6):
        ...
        if not User.objects.filter(username=username).exists():
            u = User.objects.create_user(
                username, f"amp{i}@example.com", password, role=User.Role.AMPAYER
            )
            UserProfile.objects.create(
                user=u,
                certification_level="Level A",
    ...
    for i in range(1, 3):
        ...
        if not User.objects.filter(username=username).exists():
            u = User.objects.create_user(
                username, f"score{i}@example.com", "password123", role=User.Role.SCORER
            )
...
def create_teams(season, cat_major, cat_junior):
    ...
    for name in names_major:
        teams.append(
            Team.objects.create(
                category=cat_major, name=name, short_name=name[:3].upper()
            )
        )
    ...
    for name in names_junior:
        teams.append(
            Team.objects.create(
                category=cat_junior, name=name, short_name=name[:3].upper()
            )
        )
...
def create_stadiums():
    ...
    for i in range(1, 4):
        stadiums.append(
            Stadium.objects.create(name=f"Estadio Municipal {i}", address=f"Calle {i}")
        )
    return stadiums
